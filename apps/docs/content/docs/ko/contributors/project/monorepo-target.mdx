...
apps/
  ...
  mcp-gateway/         # Multi-backend MCP gateway shell
  worker/              # Async worker shell
  web/                 # Next.js dashboard + API scaffold
...
packages/
  ...
  backend-blockbench/  # Dispatcher-backed blockbench adapter
  backend-engine/      # Clean-room engine adapter scaffold
deploy/
  docker-compose.yml   # Multi-service scaffold (web/gateway/worker/db)
config/
...
2. 백엔드 선택은 `packages/backend-core` 인터페이스를 통해서만 라우팅합니다.
