...
import type { TextureCompositionRegion } from '@ashfox/engine-core';
import {
...
const surfacePattern = (
  ...
  const sampleKeys = Array.from(
    ...
    (_, index) =>
      `${bounds.x + index % bounds.width},${
        bounds.y + Math.floor(index / bounds.width)
      }`
  );
  return {
    seedKey,
    ...
    toneField: {
      appearance: {
        version: 1,
        seed: { kind: 'explicit', value: seedKey },
        semanticOwnerKey: seedKey,
        domain: 'neutral',
        texture: {
          kind: 'mottle',
          scale: 'broad',
          density: 'balanced',
          contrast: tonePolicy === 'focal' ? 'bold' : 'subtle'
        },
        decoration: tonePolicy === 'focal' ? 'focal' : 'body',
        geometry: 'mass',
        faceDirection: 'north',
        faceAspect: 'anterior',
        plane: 0,
        ...
        frame: {
          ...
          forwardRange: { minimum: 0, maximum: 1 }
        },
        tonePolicy
      },
      bounds,
      shadowKeys: sampleKeys.filter((_, index) => index % 5 === 0).sort(),
      lightKeys: sampleKeys.filter((_, index) => index % 7 === 3).sort()
    },
    origin,
    bounds
  };
