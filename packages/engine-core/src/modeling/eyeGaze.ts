...
import type { EyeFeaturePartSpec } from './partContract';
import {
  surfaceFeatureAxes,
  surfaceFeaturePixels,
  type SurfaceFeaturePixel
} from './surfaceFeature';
import type { Axis, CellKey } from './types';
...
export const centeredEyePupilBias = (
  ...
  if (eye.size[0] % 2 === 1 || intent.symmetry.kind !== 'bilateral') {
    return 0;
  }
  ...
  const worldDirection = Math.sign(frame.planeTwice as number - centerTwice);
