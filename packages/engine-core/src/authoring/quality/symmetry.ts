...
import type { FeaturePartSpec } from '../../modeling/part';
import {
  readPartRecipe
...
export const evaluateSymmetryQuality = (
  ...
  const centered = document.intent.symmetry.kind === 'bilateral'
    ? profile.slots.filter((slot) => slot.symmetry.kind === 'centered')
    : [];
