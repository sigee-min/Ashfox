...
export const readAuthoringRestPose = (
  ...
  if (
    ...
    addIssue(
      ...
      '{kind:"canonical-neutral",mode:standing|rolling|supported|none|free-explicit}'
    );
    return null;
  }
  ...
  if (!derived) {
    addIssue(
      ...
      'grounded feet only for standing, grounded wheels only for rolling, grounded bases only for supported, and no grounded support for none or free-explicit'
    );
    return null;
  }
  if (mode !== derived) {
