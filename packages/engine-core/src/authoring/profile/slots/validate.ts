...
import { slotGraphHasCycle } from '../../catalog/rules';
import {
 ...
 * A centered or asymmetric parent is shared by exact ID; a paired parent is
 * matched through its pair ID on the same semantic side as the child. This
 * lets a globally asymmetric asset own an explicitly reflected local pair.
 */
const pairedParentSignature = (
  ...
  for (const parentSlotId of slot.parentSlotIds) {
    ...
    if (parent.symmetry.kind === 'asymmetric') {
      signature.push(`asymmetric:${parent.slotId}`);
      continue;
    }
...
const validateSpatialContracts = (
  ...
  if (projectSymmetry?.kind === 'bilateral') {
    ...
    const centered = slots.find((slot) => slot.symmetry.kind === 'centered');
    if (centered) {
      addIssue(
        issues,
        `slots.${centered.slotId}.symmetry`,
        'A globally asymmetric project cannot declare centered ownership.',
        '{kind:"asymmetric"}'
      );
    }
    const paired = slots.find((slot) => slot.symmetry.kind === 'paired');
    if (paired && projectSymmetry.pairPlaneTwice === undefined) {
      addIssue(
        issues,
        `slots.${paired.slotId}.symmetry`,
        'Paired slots require an explicit local pair plane.',
        'project symmetry {kind:"asymmetric",pairPlaneTwice:<integer>}'
      );
    }
  }
  for (const slot of slots) {
  ...
  if (intent?.grounding === 'none' && groundedSupport) {
    addIssue(
      issues,
      'slots',
      'Contact-free project intent cannot declare grounded support.',
