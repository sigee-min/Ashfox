...
const validateSupportedSurfaces = (
  ...
  for (const obligation of obligations.values()) {
    ...
    if (obligation.cardinality === 'single') {
    ...
    const expected = obligation.growth === 'up'
      ? 'above'
      : obligation.growth === 'forward'
        ? 'front'
        : obligation.growth === 'rearward'
          ? 'rear'
          : obligation.growth === 'left'
            ? 'left'
            : obligation.growth === 'right'
              ? 'right'
              : null;
    const opposite = obligation.growth === 'up'
      ? 'below'
      : obligation.growth === 'forward'
        ? 'rear'
        : obligation.growth === 'rearward'
          ...
          : obligation.growth === 'left'
            ...
            : obligation.growth === 'right'
              ? 'left'
              : null;
    const invalidGrowth = group.find((slot) => {
      if (obligation.growth === 'outward') {
    ...
    if (invalidGrowth) {
      addIssue(
        issues,
        `slots.${invalidGrowth.slotId}.spatialRelations`,
        `${obligation.role} span does not realize its sealed "${obligation.growth}" growth.`,
        obligation.growth === 'outward'
