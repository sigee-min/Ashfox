...
export const readAuthoringRestPose = (
  ...
  if (
    ...
    addIssue(
      ...
      '{kind:"canonical-neutral",mode:standing|supported|airborne|free}'
    );
    return null;
  }
  ...
  if (!derived) {
    addIssue(
      ...
      'grounded feet only for standing, grounded bases only for supported, and no grounded support for airborne or free'
    );
    return null;
  }
  if (mode !== derived) {
