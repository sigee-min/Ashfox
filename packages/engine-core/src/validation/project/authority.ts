...
import { projectIntentReader } from '../../project/intent';
...
import {
  compilerOwnedFieldsEqual,
  DEFAULT_INTENT_VALIDATION_COMPUTATION,
  readIntentProgramValidationEvidence,
...
const validateIntent = (
  ...
  const result = projectIntentReader.read(document);
  if (result.ok) return;
...
const validateIntentPrograms = (
  ...
  for (const [path, program] of [
    ...
    if (issue || !parsed.canonical || parsed.hash !== program.hash) {
      add({
        code: 'document.invalid_intent',
        severity: 'error',
        ...
        path: issue
          ...
          : `${path}.hash`,
        fix: 'Have the Agent submit the complete source, then compile its verified source digest.'
      });
      continue;
    }
    const receiptIssues = validateIntentProgramReceipt(program, {
      source: program.source,
      semanticCanonical: parsed.canonical
    });
    for (const receiptIssue of receiptIssues) {
      add({
        code: 'document.invalid_intent',
        severity: 'error',
        message: receiptIssue.message,
        path: `${path}.${receiptIssue.path}`,
        fix: 'Propose the source again to create a current compilation receipt.'
      });
    }
  }

  // The Agent can promote a staged program immediately after its compile
  // decision. Imports must not admit a source the compiler cannot materialize.
  ...
  if (parseIssue || !parsed.canonical || parsed.hash !== proposal.hash) return;
  const materialized = computation.materialize(document, proposal);
  if (materialized.ok) {
    const outputIssue = validateIntentProgramReceipt(proposal, {
      source: proposal.source,
      semanticCanonical: parsed.canonical,
      outputDigest: computation.outputDigest(materialized.document)
    }).find((issue) => issue.path === 'receipt.outputDigest');
    if (outputIssue) {
      add({
        code: 'document.invalid_intent',
        severity: 'error',
        message: outputIssue.message,
        path: 'intentProgramProposal.receipt.outputDigest',
        fix: 'Propose the source again before compiling it.'
      });
    }
    return;
  }
...
const validateCompilerAuthority = (
  ...
  if (!document.intentProgram) {
    if (
      ...
      add({
        code: 'document.invalid_intent',
        severity: 'error',
        message: 'A semantic intent, profile, or generated model requires one compiled Intent Program source.',
        path: 'intentProgram',
  ...
  const evidence = readIntentProgramValidationEvidence(
    document,
    attestation
  );
  if (evidence) return;
  ...
  if (!compilerOwnedFieldsEqual(document, materialized.document)) {
    add({
      code: 'document.invalid_intent',
      severity: 'error',
      message: 'Stored project output does not exactly match its Intent Program compilation.',
      path: 'intentProgram',
      fix: 'Recompile the authoritative source; direct intent, profile, scene, texture, material, and animation edits are not permitted.'
    });
  }
  const parsed = parseIntentProgram(document.intentProgram.source);
  const outputIssue = parsed.canonical
    ? validateIntentProgramReceipt(document.intentProgram, {
        source: document.intentProgram.source,
        semanticCanonical: parsed.canonical,
        outputDigest: computation.outputDigest(document)
      }).find((issue) => issue.path === 'receipt.outputDigest')
    : undefined;
  if (outputIssue) {
    add({
      ...
      severity: 'error',
      message: outputIssue.message,
      path: 'intentProgram.receipt.outputDigest',
      fix: 'Recompile the authoritative source from a fresh Agent proposal.'
    });
  }
};
...
export const validateDocument = (
  ...
  validateIntentPrograms(document, add, computation);
  validateCompilerAuthority(document, add, attestation, computation);
