...
const sourcePathForPartId = (partId: string): string => {
  ...
  if (partId.startsWith('support.')) return 'rest';
  return 'asset';
};
...
export const sourcePathForRecipeIssue = (
  ...
  if (issuePath.startsWith('modeling.materials')) return 'style.palette';
  return 'asset';
};
...
export const sourcePathForCompilerIssue = (
  ...
  if (recipePath !== 'asset') return recipePath;
  ...
  if (issuePath.includes('support') || issuePath.includes('restPose')) return 'rest';
