...
import { BlockbenchViewportRefresher } from '../../../../src/adapters/blockbench/viewport/refresh';
...

{
  ...
  const lastUpdateView: { options?: CanvasUpdateViewOptions } = {};
  withGlobals(
    {
      Canvas: {
        updateView: (options: CanvasUpdateViewOptions) => {
          updateViewCalls += 1;
          lastUpdateView.options = options;
        }
      },
      Preview: {
        selected: {
  ...
  assert.equal(lastUpdateView.options?.element_aspects?.geometry, true);
  assert.equal(lastUpdateView.options?.element_aspects?.uv, true);
