...
import type { ToolError, Capabilities } from '@ashfox/blockbench-contracts/types/internal';
import { ProjectSession } from '../../../src/session';
import { ProjectLifecycleService } from '../../../src/usecases/project/lifecycle';
import { ProjectStateBuilder } from '../../../src/domain/project/projectStateBuilder';
import type { ProjectServiceDeps } from '../../../src/usecases/project/types';
import {
...
const createHarness = (options?: {
  ...
  const projectState = new ProjectStateBuilder({
    listFormats: () => formats.listFormats(),
    getActiveFormatId: () =>
      session.snapshot().id ? formats.getActiveFormatId() : null
  });
