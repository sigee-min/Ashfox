...
import type { Capabilities, TextureUsageResult } from '@ashfox/blockbench-contracts/types/internal';
...
assert.ok(
  preflightRes.ok && (preflightRes.value.warnings?.length ?? 0) > 0
);
...
const usageId = computeTextureUsageId(toDomainTextureUsage(usage));
