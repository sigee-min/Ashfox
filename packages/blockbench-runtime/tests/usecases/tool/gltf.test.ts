...
if (stateRes.ok) {
  const mesh = stateRes.value.project.meshes?.find(
    (entry) => entry.name === 'body'
  );
  const cube = stateRes.value.project.cubes?.find(
    (entry) => entry.name === 'body_cube'
  );
  const anim = stateRes.value.project.animations?.find(
