...
import { BlockbenchProjectAdapter } from '../../../src/adapters/blockbench/project';
import {
...
import { withGlobals as withGlobalOverrides } from '../../support/globals';
...

{
  ...
  withGlobals(
    ...
    () => {
      ...
      const err = adapter.closeProject();
      assert.ok(err);
      if ('pending' in err) throw new Error('Expected a synchronous close error.');
      assert.equal(err.code, 'unknown');
