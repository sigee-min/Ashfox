...
export const buildGroups = (
  ...
  for (const cube of entry.cubes) {
    ...
    if (!resolved) {
      return atlasFailure('invalid_state', messages.cubeMissing(cube.name), {
    ...
    for (const face of cube.faces) {
      ...
      if (!expected) {
        return atlasFailure('invalid_state', messages.deriveSizeFailed(cube.name, face.face), {
          textureName: entry.name,
          cubeName: cube.name,
          face: face.face,
          dimensions: dims,
      ...
      if (width > config.width || height > config.height) {
        return atlasFailure('invalid_state', messages.uvSizeExceeds(cube.name, face.face), {
