...
const prepareBuildContext = (context: BuildContext): DomainResult<PreparedBuildContext> => {
  ...
  if (!startWidth || !startHeight) {
    return atlasFailure('invalid_payload', context.messages.resolutionPositive);
  }
  ...
  if (!maxWidth || !maxHeight) {
    return atlasFailure('invalid_payload', context.messages.maxResolutionPositive);
  }
...
const nextResolutionStep = (
  ...
  if (nextWidth > max.width || nextHeight > max.height) {
    return atlasFailure('invalid_state', messages.exceedsMax, {
      width,
      height,
      nextWidth,
      nextHeight,
