import type { SessionState, TrackedCubeFace } from '../../../session';
import type {
  BlockbenchGlobals,
  CubeFace,
  CubeFaceDirection,
...
const VALID_FACE_ROTATIONS = new Set<TrackedCubeFace['rotation']>([0, 90, 180, 270]);
  ...
  (nodes ?? []).forEach((node) => {
    ...
    if (isCube(node, globals)) {
      cubes.push({
        ...
        boxUv: node.box_uv,
        shade: node.shade,
        lightEmission: node.light_emission,
        rescale: node.rescale,
        faces: toCubeFaces(node.faces)
      });
      return;
    }
...
const toCubeFaces = (
  value: Record<string, CubeFace> | undefined
): Partial<Record<CubeFaceDirection, TrackedCubeFace>> | undefined => {
  if (!isRecord(value)) return undefined;
  const result: Partial<Record<CubeFaceDirection, TrackedCubeFace>> = {};
  for (const [key, valueFace] of Object.entries(value)) {
    if (!VALID_FACE_KEYS.has(key as CubeFaceDirection) || !isRecord(valueFace)) continue;
    const direction = key as CubeFaceDirection;
    const rotation = VALID_FACE_ROTATIONS.has(valueFace.rotation as TrackedCubeFace['rotation'])
      ? (valueFace.rotation as TrackedCubeFace['rotation'])
      : undefined;
    const cullface = VALID_FACE_KEYS.has(valueFace.cullface as CubeFaceDirection)
      ? (valueFace.cullface as CubeFaceDirection)
      : undefined;
    const uv = toOptionalVec4(valueFace.uv);
    result[direction] = {
      enabled: valueFace.enabled !== false,
      ...(typeof valueFace.texture === 'string' || valueFace.texture === false || valueFace.texture === null
        ? { texture: valueFace.texture }
        : {}),
      ...(uv ? { uv } : {}),
      ...(rotation === undefined ? {} : { rotation }),
      ...(cullface ? { cullface } : {}),
      ...(typeof valueFace.tint === 'number' ? { tintIndex: valueFace.tint } : {}),
      ...(typeof valueFace.material_name === 'string' ? { materialInstance: valueFace.material_name } : {})
    };
  }
  return Object.keys(result).length > 0 ? result : undefined;
};

const toOptionalVec4 = (value: unknown): [number, number, number, number] | undefined => {
  if (!Array.isArray(value) || value.length < 4) return undefined;
  if (!value.slice(0, 4).every((entry) => typeof entry === 'number')) return undefined;
