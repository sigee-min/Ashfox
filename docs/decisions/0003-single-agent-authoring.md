...
One AI IDE agent authors the asset in the browser. Quality per token depends on
...
- Web UI and AI IDE use the same deterministic commands.
...
- A new AI IDE step runs only when new state or rendered evidence can change the
  next command.
...
- Visual judgment remains with AI IDE and the creator.
