...
Status: accepted; v1 is a hard cut with no predecessor compatibility contract.
...
exactly the current explicit v1 contract.

## Authority model

An `Archetype` is the broad body plan. It owns semantic body slots,
...
```mermaid
flowchart LR
  I["Intent and reference provenance"] --> P["Canonical v1 profile"]
  A["One Archetype"] --> P
...
The profile contains one explicit `{id, version: 1}` archetype reference,
explicit v1 specialist references, authority claims, archetype slot
assignments, attachment or motion bindings, and a routing snapshot. String
...
required criterion has a claim with an authority v1 reference, matching
...
- one concrete host slot allowed by that port; and
...
A motion binding atomically identifies one selected specialist v1 reference,
...
Only `compatibilityEvaluator.ts` interprets compatibility clauses. The v1
...
`Assembly` is intentionally not implemented in v1. There is no Assembly type,
