...
function loadToolData() {
  const tmpDir = path.join(repoRoot, '.bbmcp', 'tmp');
...
function generateToolReferenceIndex(locale) {
  ...
  const description =
    locale === 'ko'
      ? '카테고리별 폴더 구조와 툴별 상세 페이지로 구성된 bbmcp MCP 도구 참조.'
      : 'Category-based reference with one dedicated page per bbmcp MCP tool.';
