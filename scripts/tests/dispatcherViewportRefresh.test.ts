...

{
  ...
  const stateRes = dispatcher.handle('get_project_state', { detail: 'summary' });
  assert.equal(stateRes.ok, true);
  assert.equal(refreshCalls.length, 0);

  const cubeRes = dispatcher.handle('add_cube', {
    name: 'body_main',
    from: [-4, 0, -4],
    to: [4, 8, 4]
  });
  ...
  const poseRes = dispatcher.handle('set_frame_pose', {
    clip: 'idle',
    frame: 0,
  ...
  const badCubeRes = dispatcher.handle('add_cube', {
    name: '',
    from: [0, 0, 0],
    to: [1, 1, 1]
  });
