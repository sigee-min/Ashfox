...
export const TEXTURE_WORKFLOW_INSTRUCTIONS = [
  'Textures are low-level only: assign_texture -> paint_faces. UVs are managed internally (no manual UV tools).',
  'ensure_project auto-creates a texture named after the project when none exists.',
  'paint_faces is strict single-write: send exactly one target (cubeId/cubeName + face) and one op.',
...
export const SERVER_TOOL_INSTRUCTIONS = [
  ...
  'paint_faces requires one target (cubeId/cubeName + face) and one op; default coordSpace is face.',
