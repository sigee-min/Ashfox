...
export const textureOpSchema: JsonSchema = {
  ...
  properties: {
    op: {
      ...
      description:
        'Drawing operation. Coordinates are in source-canvas pixels (not UV pixels). Prefer presets for large textures.'
    },
    ...
    target: {
      ...
      properties: {
        cubeIds: {
          ...
          minItems: 1,
          items: { type: 'string' },
          description: 'Limit painting to these cube ids.'
        },
        cubeNames: {
          type: 'array',
          minItems: 1,
          items: { type: 'string' },
          description: 'Limit painting to these cube names.'
        },
        faces: {
          type: 'array',
          minItems: 1,
